...
def convnet(inputs):
    ...
    W_conv1 = tf.get_variable("W_conv1", [5, 5, 1, 32], initializer=tf.random_normal_initializer(0, 0.1))
    b_conv1 = tf.get_variable("b_conv1", [32], initializer = tf.constant_initializer(0.1))
    W_conv2 = tf.get_variable("W_conv2", [5, 5, 32, 64], initializer=tf.random_normal_initializer(0, 0.1))
    b_conv2 = tf.get_variable("b_conv2", [64], initializer = tf.constant_initializer(0.1))
    W_fc1 = tf.get_variable("W_fc1", [7 * 7 * 64, 784], initializer=tf.random_normal_initializer(0, 0.1))
    b_fc1 = tf.get_variable("b_fc1", [784], initializer = tf.constant_initializer(0.1))
...
#    W_fc1 = [weight_variable([7 * 7 * 64, 784], i + "W_fc1") for i in layer_names]
#    b_fc1 = [bias_variable([784], i + "b_fc1") for i in layer_names]
